...
BasicCity::BasicCity(std::string label, int population, std::shared_ptr<std::vector<Route>> routes, std::shared_ptr<std::random_device> device)
    : Node(label), population(population), carFactory(routes, device)
{
