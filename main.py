...
def main():
    ...
    until = int(8 / 24 * steps_per_day)
    for i in range(until):
        if i % 100 == 0:
            cars_per_edge.append(simulation.get_n_cars_per_edge())
            times.append(i / steps_per_day * 24)
...
#     cars_per_edge = cars_per_edge[start:end]
#
# #   STABILIZATION PROOF:
#     for i in range(len(cars_per_edge[0])):
#         plt.plot(times, [cars_per_edge[j][i] for j in range(len(cars_per_edge))])
#     plt.xlabel("Hour of the day")
#     plt.ylabel("Amount of cars on road")
#     plt.savefig("stabilization_proof.svg")
