...
if TYPE_CHECKING:
    from traffic_jelly.visualization import GameRoadState
    from traffic_jelly.visualization import GameState


class Camera(metaclass=ABCMeta):
    @abstractmethod
    def get_x_center(self, state: 'GameState', road_state: 'GameRoadState') -> float: pass
...
class StationaryCamera(Camera):
    ...
    def get_x_center(self, state: 'GameState', road_state: 'GameRoadState') -> float:
...
class CarCamera(Camera):
    ...
    def get_x_center(self, state: 'GameState', road_state: 'GameRoadState') -> float:
        ...
        # if self.__time_step < road_state.time_step:
        #     self.__time_step = road_state.time_step
        #     self.__prev_x = self.__next_x
        #     self.__next_x = self.__simulation.get_car(self.__car_id).x
        # alpha = road_state.time % state.delta_time / state.delta_time
        # beta = 1 - alpha
